        ...
        if (equip.getType() == 1) { // See Item.java
            ...
            if (GameConstants.isPotentialScroll(scrollId.getItemId()) || GameConstants.isEquipScroll(scrollId.getItemId()) || GameConstants.isSpecialScroll(scrollId.getItemId()) || Randomizer.nextInt(100) <= success) {
                    ...
                    default: {
                        if (GameConstants.isChaosScroll(scrollId.getItemId())) {
                            ...
                            if (Randomizer.nextInt(100) > chanc) {
                                if (ItemFlag.SHIELD_WARD.check(nEquip.getFlag())) {
                                    chr.dropMessage(5, "Successfully used equipment to enhance scrolls.");
                                } else {
                                    return null;
                                }
                            } else {
                            ...
                            if (nEquip.getState() <= 17 && (scrollId.getItemId() / 100 == 20497)) {
                                final int chanc = (scrollId.getItemId() == 2049700 ? 100 : 80) + added;
                                if (Randomizer.nextInt(100) > chanc) {
                                    if (ItemFlag.SHIELD_WARD.check(nEquip.getFlag())) {
                                        chr.dropMessage(5, "Successfully used the latent ability scroll.");
                                    } else {
                                        return null;
                                    }
                                } else {
                                ...
                                if (Randomizer.nextInt(100) > chanc) {
                                    if (ItemFlag.SHIELD_WARD.check(nEquip.getFlag())) {//使用潛能捲軸時，保護裝備不會被損壞
                                        chr.dropMessage(5, "The scroll lights up, but the item winds up as if nothing happened.");
                                    } else {
                                        return null;
                                    }
                                } else {
                ...
                if (!ws && !GameConstants.isInnocence(scrollId.getItemId()) && !GameConstants.isCleanSlate(scrollId.getItemId()) && !GameConstants.isSpecialScroll(scrollId.getItemId()) && !GameConstants.isEquipScroll(scrollId.getItemId()) && !GameConstants.isPotentialScroll(scrollId.getItemId())) {
                    if (ItemFlag.SLOTS_PROTECT.check(nEquip.getFlag())) { //使用安全盾牌捲軸，失敗時不會減少裝備可使用捲軸次數
                        chr.dropMessage(5, "The scroll lights up, but the item winds up as if nothing happened.");
                    } else {
                        nEquip.setUpgradeSlots((byte) (nEquip.getUpgradeSlots() - 1));
                    }
                ...
                if (Randomizer.nextInt(99) < curse) {
                    if (ItemFlag.SHIELD_WARD.check(nEquip.getFlag())) {//使用一般捲軸時，保護裝備不會被損壞
                        chr.dropMessage(5, "Due to the effect of the scroll, the equipment has not undergone any changes.");
                    } else {
                        return null;
                    }
                }
