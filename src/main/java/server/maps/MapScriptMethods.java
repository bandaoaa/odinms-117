...
public class MapScriptMethods {
    ...
    public static void startScript_FirstUser(MapleClient c, String scriptName) {
        ...
        switch (onFirstUserEnter.fromString(scriptName)) {
            case PTtutor000:
                c.getPlayer().getMap().startMapEffect("Please click on the npc to start your adventure", 5120016);
                break;
            case dojang_Eff: {
    ...
    public static void startScript_User(final MapleClient c, String scriptName) {
        ...
        switch (onUserEnter.fromString(scriptName)) {
            ...
            case map_913070000: {//MIHILE MAP
                c.getSession().write(UIPacket.IntroDisableUI(true));
                c.getSession().write(UIPacket.IntroLock(true));
                c.getPlayer().dropMessage(-1, "Mr.Limbert's General Store");
                c.getPlayer().dropMessage(-1, "Month 3, Day 4");
                c.getSession().write(CField.facialExpression2(6, 5000)); //F6
                c.getPlayer().dropMessage(-1, "YOU LITTLE HOODLUM - Mr.Limbert");//Little Hoodlum
                c.getSession().write(CField.facialExpression2(6, 5000)); //F6
                c.getPlayer().dropMessage(-1, "DON'T CLUTTER UP THE STORE! MOVE THAT BOX! -Mr.Limbert");//Little Hoodlum
                c.getSession().write(CField.facialExpression2(6, 5000)); //F6
                c.getPlayer().dropMessage(-1, "Sorry Mr.Limbert! -Mihile");//Little Hoodlum
                c.getSession().write(CField.facialExpression2(4, 5000)); //FACIAL EXPRESSION F4
                c.getPlayer().dropMessage(-1, "Why is this guy staring at me? -Mihile");//Little Hoodlum
                c.getSession().write(CField.facialExpression2(6, 5000)); //FACIAL EXPRESSION F6
                c.getSession().write(UIPacket.IntroLock(true));
                c.getSession().write(UIPacket.IntroDisableUI(true));
                break;
            }
