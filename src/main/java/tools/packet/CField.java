...
public class CField {
    ...
    public static class EffectPacket {
        ...
        public static byte[] showBuffeffect(int cid, int skillid, int effectid, int playerLevel, int skillLevel, byte direction) {
            ...
            if (effectid == 2 && skillid == 31111003) { //血腥烏鴉
        ...
        */
        public static byte[] ShowSoulSteelEffect(int cid, int buffid, int skillid, int skillLevel) {
            ...
            mplew.writeInt(cid);
            mplew.write(3);
            mplew.writeInt(buffid); //1=攻擊反射 2=無敵 3=防禦力Buff 4=攻擊力Buff
            mplew.writeInt(-1);
            mplew.writeInt(skillid);
    ...
    public static class UIPacket {
        ...
        */
        ...
        打開楓之高校積分介面
        */
        public static byte[] sendFourPillarsOfHeaven(int points, boolean viewonly) {
    ...
    public static class SummonPacket {
        ...
        public static byte[] spawnSummon(MapleSummon summon, boolean animated) {
            ...
            mplew.write(summon.getSkill() == 32111006 || summon.getSkill() == 33101005 ? 5 : 4); //咆哮 甦醒
            if (summon.getSkill() == 35121003 && summon.getOwner().getMap() != null) { //戰鬥機器 : 巨人錘
            ...
            mplew.write(summon.getSkill() == 4341006 && chr != null ? 1 : 0); //幻影替身
            if (summon.getSkill() == 4341006 && chr != null) {
                PacketHelper.addCharLook(mplew, chr, true);
            }
            if (summon.getSkill() == 35111002) { //磁場
                mplew.write(0);
            }
    ...
    public static class NPCPacket {
        ...
        */
        public static byte[] getSlideMenu(int npcid, int type, int lasticon, String sel) {
            ...
            mplew.write(GameConstants.GMS ? 0x11 : 0x10);
            mplew.write(0);
        ...
        */
        ...
        楓之高校相關內容
        */
        public static byte[] updateFriendship(MapleCharacter chr) {
            MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();

            mplew.writeShort(SendPacketOpcode.UPDATE_FRIENDSHIP.getValue());
            PacketHelper.addPillarData(mplew, chr);
    ...
    PVP成績介面
    */
    public static byte[] getPVPResult(List<Pair<Integer, MapleCharacter>> flags, int exp, int BP, int winningTeam, int playerTeam) { //Flag_R_1 to 0, etc
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    public static class InteractionPacket {
        ...
        */
        public static byte[] getTradeCancel(final byte UserSlot, final int unsuccessful) { //0 = canceled 1 = invent space 2 = pickuprestricted
            ...
            mplew.write(GameConstants.GMS ? 18 : 0xA);
            mplew.write(UserSlot);
            mplew.write(unsuccessful == 0 ? (GameConstants.GMS ? 7 : 2) : (unsuccessful == 1 ? 8 : 9)); //取消交換申請時，發送2，為拒絕提示
    ...
    */
    ...
    public static byte[] getQuickMove(List<QuickMoveEntry> quickmoves) {
        ...
        for (QuickMoveEntry q : quickmoves) {
            ...
            mplew.writeInt(q.getNpcId());
            mplew.writeInt(q.getIcon()); // type
            mplew.writeInt(q.getLevelLimit()); //level needed
            mplew.writeMapleAsciiString(q.getDesc()); // Description upon hovering
        }
        // 0: Ariant guy
        // 1: Monster Park Taxi
    ...
    public static byte[] spawnPlayerMapobject(MapleCharacter chr) {
        ...
        if (chr.getBuffedValue(MapleBuffStat.SOARING) != null) { //飛翔技能
    ...
    */
    public static byte[] getScrollEffect(int chr, ScrollResult scrollSuccess, boolean legendarySpirit, boolean whiteScroll) {
        ...
        mplew.writeInt(chr);
        mplew.write(scrollSuccess == ScrollResult.SUCCESS ? 1 : scrollSuccess == ScrollResult.CURSE ? 2 : 0); // 0 = fail, 1 = success, 2 = boom, 3 = cannot be used on this item
    ...
    public static byte[] pvpAttack(int cid, int playerLevel, int skill, int skillLevel, int speed, int mastery, int projectile, int attackCount, int chargeTime, int stance, int direction, int range, int linkSkill, int linkSkillLevel, boolean movementSkill, boolean pushTarget, boolean pullTarget, List<AttackPair> attack) {
        ...
        if (GameConstants.isIceKnightSkill(skill)) {//PVP冰騎士相關
            mplew.write(1);
            mplew.writeInt(skill);
            mplew.write(1);
        } else {
    ...
    */
    ...
    public static byte[] addAttackInfo(final int type, final int cid, final int tbyte, final int skill, final int level, final int display, final byte speed, final List<AttackPair> damage, final int lvl, final byte mastery, final byte unk, final int charge, final Point pos, final int ultLevel) {
        ...
        if (skill > 0 || type == 3) { // magic
            mplew.write(level);
            mplew.writeInt(skill == 1092 ? 10001092 : skill == 1094 ? 10001094 : skill); //冒險家職業群技能ID長度會導致其他玩家閃退
        } else {
            mplew.write(0);
        }
    ...
    public static byte[] spawnMist(final MapleMist mist) {
        ...
        mplew.writeLong(0);
        mplew.writePos(mist.getPosition()); //地獄爆發座標識別
        mplew.writeZeroBytes(50);
        return mplew.getPacket();
    }
