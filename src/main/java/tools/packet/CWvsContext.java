...
public class CWvsContext {
    ...
    */  
    ...
    /*
     卡牌審判   
    */    
        public static byte[] giveCard(int buffid, int bufflength, Map<MapleBuffStat, Integer> statups, MapleStatEffect effect) {
            MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
            mplew.writeShort(SendPacketOpcode.GIVE_BUFF.getValue());
            PacketHelper.writeBuffMask(mplew, statups);
            int extra = 0;
    ...
    /*
     靈魂竊取  
    */  
        public static byte[] soulSteel(int buffid, int bufflength, int buffeffect, Map<MapleBuffStat, Integer> statups) {
            MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
            mplew.writeShort(SendPacketOpcode.GIVE_BUFF.getValue());
            PacketHelper.writeBuffMask(mplew, statups);
    ...
    */   
        ...
        public static byte[] giveForeignPirate(Map<MapleBuffStat, Integer> statups, int duration, int cid, int skillid) {
            ...
            boolean infusion = skillid == 5121009 || skillid == 15111005; // 最終極速 最終極速
            MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        ...
        */
        public static byte[] giveBuff(int buffid, int bufflength, Map<MapleBuffStat, Integer> statups, MapleStatEffect effect) {
            ...
            for (Entry<MapleBuffStat, Integer> stat : statups.entrySet()) {
                ...
                if (effect.isMonsterRiding() || stat.getKey() == MapleBuffStat.MORPH) { //使用騎獸和變身技能
                    buffid = 0;
                }
                if (stat.getKey().canStack()) {
                    ...
                    if (stat.getKey() == MapleBuffStat.SPIRIT_SURGE || stat.getKey() == MapleBuffStat.SPIRIT_LINK || buffid == 23101003) { //靈魂灌注
            ...
            //鬥氣爆發BUFF圖示
            mplew.writeShort((buffid == 1111002 || buffid == 11111001) ? 1000 : 0);
            mplew.writeShort(0);
            mplew.write(0);
            mplew.write(0);
            mplew.write(effect != null && effect.isShadow() ? 1 : 3); // Test

            if (isAura || statups.containsKey(MapleBuffStat.BODY_BOOST) || statups.containsKey(MapleBuffStat.JUDGMENT_DRAW)) {
                mplew.writeInt(0);
            }
        ...
        public static byte[] giveDebuff(MapleDisease statups, int x, int skillid, int level, int duration) {
            ...
            //mplew.write(1);
            //mplew.writeZeroBytes(60);
            //System.out.println(HexTool.toString(mplew.getPacket())); //DeBuff於dos窗Hex訊息
            return mplew.getPacket();
        }
...
mplew.writeInt(0);
    ...
    public static class ExpeditionPacket {
        ...
        public static byte[] expeditionUpdate(final int partyIndex, final MapleParty party) {
            ...
            PartyPacket.addPartyStatus(-1, party, mplew, false, false);
...
mplew.writeZeroBytes(100);
    ...
    */
    ...
    寶盒的護佑相關內容
    */
    public static byte[] updateCoreAura(MapleCharacter chr, int type) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        ...
        mplew.writeInt(-1);
        PacketHelper.addCoreAura(mplew, chr, type);
