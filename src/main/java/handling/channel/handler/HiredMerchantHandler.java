...
public class HiredMerchantHandler {
    ...
    public static boolean UseHiredMerchant(final MapleClient c, final boolean packet) {
        if (c.getPlayer().getMap() != null && c.getPlayer().getMap().allowPersonalShop()) {
            ...
            switch (state) {
                case 1:
                    c.getPlayer().dropMessage(1, "Please claim your items from @hireditems first.");
                    break;
                case 0:
    ...
    public static void MerchantItemStore(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        if (operation == 29 || operation == 30) { // Request, Take out
            requestItems(c, operation == 29);
        } else if (operation == 32) { // Exit
            c.getPlayer().setConversation(0);
        }
    }
    ...
    private static void requestItems(final MapleClient c, final boolean request) {
        ...
        if (request && days > 0 && percentage > 0 && pack.getMesos() > 0 && fee > 0) {
            c.getSession().write(PlayerShopPacket.merchItemStore((byte) 41, days, fee));
            return;
        }
        if (fee < 0) { // impossible
        ...
        if (c.getPlayer().getMeso() < fee) {
            c.getSession().write(PlayerShopPacket.merchItem_Message(38));
            return;
        }
        if (!check(c.getPlayer(), pack)) {
            c.getSession().write(PlayerShopPacket.merchItem_Message(39));
            return;
        }
        if (deletePackage(c.getPlayer().getAccountID(), pack.getPackageid(), c.getPlayer().getId())) {
            if (fee > 0) {
