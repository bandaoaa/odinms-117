...
//            if ((type != -1) && (damage > 0)) {
    ...
    public static final void closeRangeAttack(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr, boolean energy) {
        ...
        boolean mirror = chr.getBuffedValue(MapleBuffStat.SHADOWPARTNER) != null;
    ...
    public static final void rangedAttack(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
        ...
        if ((!AOE) && (chr.getBuffedValue(MapleBuffStat.SOULARROW) == null) && (!noBullet)) {
            ...
            if (c.getPlayer().getInventory(MapleInventoryType.USE).getItem(attack.slot).getItemId() == 2070018) {
                Item bullet = (Item) c.getPlayer().getInventory(MapleInventoryType.USE).getItem(attack.slot);
                short quantity = bullet.getQuantity();
                if (quantity < 15) {
                    MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
                    MapleInventoryManipulator.removeFromSlot(c, MapleInventoryType.USE, attack.slot, (short) (-ii.getSlotMax(bullet.getItemId()) + quantity), false, true);
                }
            }
