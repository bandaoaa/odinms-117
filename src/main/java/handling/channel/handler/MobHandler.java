...
public class MobHandler {
    ...
    public static final void MoveMonster(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
        ...
        try {
            ...
            FileoutputUtil.outputFileError("MovementLog.txt", e);
            FileoutputUtil.log("MovementLog.txt", "MOBID " + monster.getId() + ", AIOBE Type2:\n" + slea.toString(true));
            return;
        }
        if ((res != null) && (chr != null) && (res.size() > 0)) {
            ...
            if (slea.available() != 36L) {
                FileoutputUtil.log("PacketLog.txt", "slea.available != 35 (movement parsing error)\n" + slea.toString(true));
                return;
            }
