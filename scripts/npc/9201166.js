...
function action(mode, type, selection) {
    ...
    switch (status) {
        case 0:
            if (cm.getPlayer().getQuestNAdd(Packages.server.quest.MapleQuest.getInstance(28767)).getStatus() > 0 || cm.getPlayer().getQuestNAdd(Packages.server.quest.MapleQuest.getInstance(28748)).getStatus() < 1) {
                cm.dispose();
                return;
            }
            if (cm.getPlayer().getPosition().x < 1100 || cm.getPlayer().getPosition().x > 1400 || cm.getPlayer().getPosition().y < -700) {
            ...
            break;
        case 1:
            cm.sendNextS("喲，這不是關於克拉齊亞的資訊麼。我看看……", 3);
            break;
        case 2:
            cm.getClient().sendPacket(Packages.tools.packet.EtcPacket.UIPacket.getDirectionInfo(1, 500));
            break;
        case 3:
            cm.getClient().sendPacket(Packages.tools.packet.EtcPacket.UIPacket.getDirectionInfo(0, 307));
            cm.getClient().sendPacket(Packages.tools.packet.EtcPacket.UIPacket.getDirectionInfo(1, 2000));
            break;
        case 4:
            cm.getClient().sendPacket(Packages.tools.packet.MaplePacketCreator.getTopMsg("破壞資訊完成"));
            Packages.server.quest.MapleQuest.getInstance(28767).forceStart(cm.getPlayer(), 0, null);
            cm.getClient().sendPacket(Packages.tools.packet.EtcPacket.UIPacket.getDirectionInfo(1, 2000));
            break;
        case 5:
            cm.sendNextS("哈哈！做了一件好事~", 3);
            break;
        case 6:
        ...
        case 8:
            ...
            cm.getClient().sendPacket(Packages.tools.packet.EtcPacket.UIPacket.getDirectionInfo(1, 2500));
            break;
        case 9:
        ...
        case 10:
            ...
            break;
        case 11:
            cm.spawnNPCRequestController(9201174, 1133, -611, 1);
            cm.getClient().sendPacket(Packages.tools.packet.EtcPacket.UIPacket.getDirectionInfo(1, 2000));
            break;
        case 12:
        ...
        case 13:
            ...
            cm.spawnNPCRequestController(9201174, 1125, -611, 0);
        ...
        case 15:
            cm.getClient().sendPacket(Packages.tools.packet.EtcPacket.NPCPacket.removeNPCController(9201174));
            cm.spawnNPCRequestController(9201174, 1135, -611, 0);
            cm.getClient().sendPacket(Packages.tools.packet.EtcPacket.UIPacket.getDirectionInfo(1, 200));
            break;
        case 16:
            cm.getClient().sendPacket(Packages.tools.packet.EtcPacket.NPCPacket.removeNPCController(9201174));
            ...
            break;
        case 17:
            cm.sendNextS("我的帳戶被黑了！早知道就弄個複雜點兒的密碼了！", 5, 9201174);
            break;
        case 18:
            cm.getClient().sendPacket(Packages.tools.packet.EtcPacket.NPCPacket.removeNPCController(9201174));
            cm.getClient().sendPacket(Packages.tools.packet.EtcPacket.UIPacket.getDirectionInfo(1, 1000));
