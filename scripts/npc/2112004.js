...
var Text = ["很感謝你來幫助我，首先我們要找到進入實驗室的入口，圖書館裏到處亂堆放的書，也許會有什麼發現也說不定。",
    ...
    "請快一些，希望時間還來得及，我們要快去救救我的愛人，茱麗葉就在前面的那個房間裏。"];
...
function action(mode, type, selection) {
    if (mode > 0) {
        ...
        for (var i = 0; i < party.size(); i++)
            if (party.get(i).getLevel() < 70 || party.get(i).getLevel() > 119 || party.get(i).getMapid() != 261000011 || party.size() < 2 || party.size() > 4) {
        ...
        cm.sendOk("拯救茱麗葉任務正在執行中，請嘗試其它頻道。");
    }
    cm.dispose();
}
