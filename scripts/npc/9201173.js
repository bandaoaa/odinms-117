...
function action(mode, type, selection) {
    ...
    switch (status) {
        case 0:
            if (cm.getPlayer().getQuestNAdd(Packages.server.quest.MapleQuest.getInstance(28765)).getStatus() > 0 || cm.getPlayer().getQuestNAdd(Packages.server.quest.MapleQuest.getInstance(28749)).getStatus() < 1) {
                cm.dispose();
                return;
            }
            if (cm.getPlayer().getPosition().x > 250 || cm.getPlayer().getPosition().y > -1391) {
        ...
        case 8:
            ...
            break;
        case 9:
            Packages.server.quest.MapleQuest.getInstance(28765).forceStart(cm.getPlayer(), 0, null);
