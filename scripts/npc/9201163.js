...
function action(mode, type, selection) {
    ...
    switch (status) {
        case 0:
            if (cm.getPlayer().getQuestNAdd(Packages.server.quest.MapleQuest.getInstance(28758)).getStatus() > 0 || cm.getPlayer().getQuestNAdd(Packages.server.quest.MapleQuest.getInstance(28748)).getStatus() < 1) {
                cm.dispose();
                return;
            }
            ...
            break;
        case 1:
            cm.sendNextS("吃我投石攻擊，葛拉特鑽孔機！", 3);
            break;
        case 2:
            cm.getClient().sendPacket(Packages.tools.packet.EtcPacket.UIPacket.getDirectionInfo(1, 500));
            break;
        case 3:
            cm.getClient().sendPacket(Packages.tools.packet.EtcPacket.UIPacket.getDirectionInfo(0, 350));
            cm.getClient().sendPacket(Packages.tools.packet.EtcPacket.UIPacket.getDirectionInfo(1, 3000));
            break;
        case 4:
            cm.getClient().sendPacket(Packages.tools.packet.MaplePacketCreator.getTopMsg("破壞葛拉特鑽孔機完成"));
        ...
        case 16:
            ...
            break;
        case 17:
            cm.sendNextS("這星球上真的是什麼都不好使！我感覺布萊薩克斯採礦公司的服務品質真是太差勁了。", 5, 9201174);
            break;
        case 18:
            cm.getClient().sendPacket(Packages.tools.packet.EtcPacket.NPCPacket.removeNPCController(9201174));
            cm.getClient().sendPacket(Packages.tools.packet.EtcPacket.UIPacket.getDirectionInfo(1, 1000));
        ...
        case 19:
            ...
            break;
        case 20:
            cm.sendNextS("誰說不是呢！哈哈哈！", 3, 9201174);
            break;
        case 21:
