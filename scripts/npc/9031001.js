...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        var selStr = "你好，我能帮你什么呢？\r\n";
        if (cm.getPlayer().getProfessionLevel(92000000) > 0) {
            selStr += "#L2##b提高#e药草采集#n等级。#l\r\n#L3#采药技术初始化。#k#l\r\n#L4##b交换药草根。#k#l";
        } else {
        ...
        if (sel == 0) {
            status = -1;
            cm.sendNext("采药是利用铲子之类的工具，采集地图上的药草的技能。把采\r\n集到的药草装在#p9031007#出售的油瓶中提炼，可以获得装备、饰品\r\n、炼金术所需的材料。");
        } else if (sel == 1) {
...
*/
            ...
            if (cm.getPlayer().getProfessionLevel(92040000) > 0) {
                ...
                status = 3;
                cm.sendYesNo("你想放弃#e采药#n？是厌倦了吗？之前积累的等级和熟练度……\r\n付出的努力和金钱……都将会变成泡影……你真的要初始化吗？");
            }
        } else if (sel == 4) {
            if (!cm.haveItem(4022023, 100)) {
                cm.sendOk("#b药草根100个#k可以交换#i2028066##b斯塔切的药草袋1个#k。请你再去搜\r\n集一些药草根。");
