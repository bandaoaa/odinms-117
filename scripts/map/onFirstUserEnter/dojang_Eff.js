...
function start(ms) {
    try {
        ...
        if (stage % 6 > 0) {
            ...
            ms.getClient().sendPacket(MaplePacketCreator.getEnergy("energy", ms.getPlayer().getDojoEnergy()));
            ...
            if (mob != null && ms.getPlayer().getMap().getMonsterById(9300183 + realstage) == null && ms.getPlayer().getMap().getMonsterById(9300216) == null) {
                ...
                ms.getClient().sendPacket(MaplePacketCreator.playSound("Dojang/start"));
                ms.getClient().sendPacket(MaplePacketCreator.showEffect("dojang/start/stage"));
                ms.getClient().sendPacket(MaplePacketCreator.showEffect("dojang/start/number/" + realstage));
            }
        }
    } catch (err) {
