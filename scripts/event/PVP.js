...
function playerEntry(eim, player) {
    ...
    if (eim.getProperty("started").equals("0")) {
        ...
        if (type != 0) {
            player.getClient().sendPacket(CField.getPVPPoints(parseInt(eim.getProperty("red")), parseInt(eim.getProperty("blue"))));
        }
        player.getClient().sendPacket(CField.getPVPScore(0, false));
    }
}
...
function addPVPScore(eim, player, score) {
    ...
    if (parseInt(eim.getProperty("ice")) == player.getId()) {
        ...
        player.getClient().sendPacket(CField.getPVPIceGage(xx));
...
function playerDead(eim, player) {
    ...
    player.getClient().sendPacket(CField.getPVPKilled(player.getName() + " has been defeated."));
    if (parseInt(eim.getProperty("ice")) == player.getId()) {
        eim.setProperty("ice", "0");
...
function championCheck(eim) {
    ...
    if (champion != null) {
        ...
        champion.getClient().sendPacket(CField.showOwnChampionEffect());
...
function updateScoreboard(eim, score) {
    ...
    if (score == true || score == null) {
        ...
        for (var i = 0; i < players.size(); i++) {
            ...
            players.get(i).getClient().sendPacket(CField.getPVPResult(c, exp, winningTeam + 1, players.get(i).getTeam() + 1));
        }
    }
}

function playerRevive(eim, player) {
    player.getClient().sendPacket(CWvsContext.clearMidMsg());
    player.getStat().heal(player);
...
function end(eim) {
    ...
    var players = eim.getPlayers();
    for (var i = 0; i < players.size(); i++) {
        players.get(i).getClient().sendPacket(CWvsContext.clearMidMsg());
