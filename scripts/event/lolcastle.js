...
function startInstance(eim) {
    if (eim.getPlayerCount() > 0) {
        ...
        while (iter.hasNext()) {
            ...
            player.getClient().sendPacket(tools.MaplePacketCreator.getClock(1200));
        }
        var map = eim.getMapInstance(mapId);
...
function playerDead(eim, player) {
    ...
    player.getClient().sendPacket(tools.MaplePacketCreator.serverNotice(1, "You lost with " + eim.getKillCount(player) + " points."));
}
...
function allMonstersDead(eim) {
    ...
    while (iter.hasNext()) {
        ...
        winner.getClient().sendPacket(tools.MaplePacketCreator.serverNotice(1, "You win with " + eim.getKillCount(winner) + " points. You will be warped out in 2 minutes."));
        winner.getClient().sendPacket(tools.MaplePacketCreator.getClock(120));
